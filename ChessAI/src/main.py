...
BOARD_WIDTH = BOARD_HEIGHT = 512#Kích thước của của bàn cờ
...
def main():
    ...
    game_state = engine.GameState()#Khởi tạo đối tượng game state
    ...
    move_log_font = p.font.SysFont("Arial", 14, False, False)#khởi tạo 1 font chữ để hiện thị thông tin lịch sử nước đi
    ...
    while running:
            ...
            # nếu event là nhấn chuột
                ...
                if not game_over:
                    ...
                    if len(player_clicks) == 2 and human_turn:  # kiểm tra người chơi nhân chuột đủ 2 lần và đến lượt của họ
                        move = engine.Move(player_clicks[0], player_clicks[1], game_state.board)#di chuyển từ nước đầu tới nước đích
        ...
        if not game_over and not human_turn and not move_undone:#nếu game ko kết thúc và đến lượt của AI
            if not ai_thinking:
                ...
                move_finder_process = Process(target=ai.findBestMove, args=(game_state, valid_moves, return_queue))# tạo 1 tiến trình tốt nhất để tìm nước đi tốt nhất cho máy tính
